...
const ToastMessageContext = createContext<ToastMessageContextProps | null>(null);
...
export default function ToastMessageProvider({ children }: PropsWithChildren) {
  ...
  const removeToastMessage = useCallback(
    (id: string) => setToastMessages(prev => prev.filter(toastMessage => toastMessage.id !== id)),
    [],
  );
  ...
  return (
    <ToastMessageContext.Provider value={{ toastMessages, removeToastMessage, showToastMessage }}>
      {children}
